...
/* ══════════════════════════════════════════════════
   ...
   Данные загружаются из /data.json (для всех)
   Админ редактирует → жмёт "Опубликовать" →
   данные пушатся в GitHub → Vercel передеплоит
   ══════════════════════════════════════════════════ */
...
const LS_KEY = 'lp_v6';
const GH_KEY = 'lp_github';
const loadLS = () => { try { const d = localStorage.getItem(LS_KEY); return d ? JSON.parse(d) : null; } catch { return null; } };
const saveLS = (d) => { try { localStorage.setItem(LS_KEY, JSON.stringify(d)); } catch {} };
const loadGH = () => { try { const d = localStorage.getItem(GH_KEY); return d ? JSON.parse(d) : { token: '', owner: '', repo: '' }; } catch { return { token: '', owner: '', repo: '' }; } };
const saveGH = (d) => { try { localStorage.setItem(GH_KEY, JSON.stringify(d)); } catch {} };

const FALLBACK = { profiles: [] };
const ghash = () => window.location.hash.replace(/^#\/?/, '');

function BIco({ b, size, radius }) {
  const sz = size || 40, r = radius || 10;
...
// ── GitHub API publish ──
async function publishToGH(gh, data) {
  const { token, owner, repo } = gh;
  if (!token || !owner || !repo) throw new Error('Заполни GitHub настройки');
  const url = `https://api.github.com/repos/${owner}/${repo}/contents/public/data.json`;
  ...
  const body = { message: '🔄 Обновление данных', content, ...(sha ? { sha } : {}) };
...
export default function App() {
  ...
  const setProfiles = (fn) => { setData(d => ({ ...d, profiles: fn(d.profiles) })); markChanged(); };
  ...
  const addProfile = () => { const id = Date.now().toString(36); setProfiles(ps => [...ps, { id, slug: id, name: 'Новый профиль', bio: '', avatar: '', theme: 'midnight', customBg: '', bgBlur: 0, bgOpacity: 0.5, cardStyle: 'glass', blocks: [] }]); setEditId(id); setTab('profile'); setEditBlock(null); flash('Создан!'); };
  ...
  const doPublish = async () => {
    if (!gh.token || !gh.owner || !gh.repo) { setShowGH(true); flash('Сначала заполни GitHub настройки'); return; }
    setPublishing(true);
    try {
  ...
  const importAll = (e) => { const f = e.target.files?.[0]; if (!f) return; const r = new FileReader(); r.onload = (ev) => { try { setData(JSON.parse(ev.target.result)); markChanged(); flash('Импорт OK!'); } catch { flash('Ошибка файла'); } }; r.readAsText(f); e.target.value = ''; };
  ...
  return (
    <>
      ...
      {/* ═══ PUBLIC PROFILE ═══ */}
      {curProfile && (() => { const p = curProfile, t = TH(p.theme); return (
        <div style={{ minHeight: '100vh', background: p.customBg ? '#000' : t.bg, display: 'flex', justifyContent: 'center', position: 'relative', fontFamily: "'Outfit',sans-serif" }}>
          ...
          <div style={{ maxWidth: 440, width: '100%', padding: '40px 20px 80px', position: 'relative', zIndex: 1 }}>
            ...
            {p.blocks.filter(b => b.on).map((b, i) => (
              <a key={b.id} href={b.url} target="_blank" rel="noopener noreferrer" className="lc" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', background: p.cardStyle === 'outline' ? 'transparent' : t.c, border: `1px solid ${t.cb}`, borderRadius: 14, marginBottom: 10, textDecoration: 'none', transition: 'all .2s', backdropFilter: p.cardStyle === 'glass' ? 'blur(12px)' : 'none', animationDelay: `${i*.05}s` }}>
                <BIco b={b} /><span style={{ fontSize: 15, fontWeight: 600, color: t.t, flex: 1 }}>{b.title}</span><span style={{ color: t.ts, fontSize: 18 }}>›</span>
              </a>
            ))}
          </div>
        ...
        <div style={{ minHeight: '100vh', background: THEMES.midnight.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 20px', fontFamily: "'Outfit',sans-serif", color: '#fff' }}>
          ...
          {profiles.map(p => (
            <div key={p.id} onClick={() => go(p.slug)} className="lc" style={{ width: '100%', maxWidth: 400, padding: '16px 20px', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, marginBottom: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 14, transition: 'all .2s' }}>
              ...
              <div><div style={{ fontWeight: 600, fontSize: 16 }}>{p.name}</div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)' }}>/{p.slug} · {p.blocks.length} ссылок</div></div>
            </div>
          ))}
      ...
      {show404 && (
        <div style={{ minHeight: '100vh', background: '#0a0a1a', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#fff', fontFamily: "'Outfit',sans-serif" }}>
          <div style={{ fontSize: 60, marginBottom: 16 }}>🤷</div><h1 style={{ fontSize: 24, marginBottom: 8 }}>Не найдено</h1><p style={{ color: 'rgba(255,255,255,0.5)', marginBottom: 24 }}>/{route}</p>
      ...
      {isAdmin && !logged && (
        <div style={{ minHeight: '100vh', background: '#0a0a1a', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Outfit',sans-serif" }}>
          <div style={{ background: '#1a1a2e', padding: 32, borderRadius: 16, width: 320, border: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <input type="password" value={pw} onChange={e => setPw(e.target.value)} placeholder="Пароль" autoFocus onKeyDown={e => { if (e.key === 'Enter') doLogin(); }} style={{ ...inpS, marginBottom: 12 }} />
              ...
              {!ep ? (
                ...
                <div style={{ display: 'flex', borderBottom: '1px solid rgba(255,255,255,0.08)', position: 'sticky', top: 0, background: '#0a0a1a', zIndex: 10 }}>
                  {[['blocks','📋 Ссылки'],['profile','👤 Профиль'],['design','🎨 Дизайн']].map(([id,lb]) => (
                    <button key={id} onClick={() => { setTab(id); setEditBlock(null); }} style={{ flex: 1, padding: '12px 6px', background: tab === id ? 'rgba(255,255,255,0.06)' : 'transparent', border: 'none', borderBottom: tab === id ? `2px solid ${et.a}` : '2px solid transparent', color: tab === id ? '#fff' : 'rgba(255,255,255,0.45)', cursor: 'pointer', fontSize: 13, fontFamily: 'inherit' }}>{lb}</button>
                  ))}
                </div>
                <div style={{ padding: 20 }}>
                  ...
                  {tab === 'blocks' && !editBlock && (<>
                    ...
                    {ep.blocks.map(b => (
                      <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 10px', background: 'rgba(255,255,255,0.04)', borderRadius: 10, marginBottom: 5, border: '1px solid rgba(255,255,255,0.06)' }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.title}</div></div>
                        <div onClick={() => updateBlock(ep.id, b.id, { on: !b.on })} style={{ width: 34, height: 18, borderRadius: 9, background: b.on ? et.a : 'rgba(255,255,255,0.15)', position: 'relative', cursor: 'pointer', transition: 'all .2s', flexShrink: 0 }}><div style={{ width: 14, height: 14, borderRadius: '50%', background: '#fff', position: 'absolute', top: 2, left: b.on ? 18 : 2, transition: 'all .2s' }} /></div>
                  ...
                  {tab === 'blocks' && editBlock && (
                    <div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        <div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginBottom: 5 }}>Название</div><input value={editBlock.title} onChange={e => updateBlock(ep.id, editBlock.id, { title: e.target.value })} style={inpS} /></div>
                        <div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginBottom: 5 }}>URL</div><input value={editBlock.url} onChange={e => updateBlock(ep.id, editBlock.id, { url: e.target.value })} style={inpS} /></div>
                        <div>
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: 5 }}>
                            {Object.entries(ICONS).map(([k, ic]) => (
                              <button key={k} onClick={() => updateBlock(ep.id, editBlock.id, { icon: k, customIcon: '' })} style={{ padding: '6px 2px', borderRadius: 7, cursor: 'pointer', textAlign: 'center', background: !editBlock.customIcon && editBlock.icon === k ? `${et.a}33` : 'rgba(255,255,255,0.05)', border: !editBlock.customIcon && editBlock.icon === k ? `2px solid ${et.a}` : '2px solid transparent', fontFamily: 'inherit' }}>
                  ...
                  {tab === 'profile' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                      ...
                      <div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginBottom: 5 }}>Slug (адрес)</div><input value={ep.slug} onChange={e => updateP(ep.id, 'slug', e.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, ''))} style={inpS} /><div style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)', marginTop: 4 }}>сайт.com/#/{ep.slug}</div></div>
                      <div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginBottom: 5 }}>Имя</div><input value={ep.name} onChange={e => updateP(ep.id, 'name', e.target.value)} style={inpS} /></div>
                      <div><div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)', marginBottom: 5 }}>Описание</div><textarea value={ep.bio} onChange={e => updateP(ep.id, 'bio', e.target.value)} rows={3} style={{ ...inpS, resize: 'vertical' }} /></div>
                    </div>
                  )}
                  ...
                  {tab === 'design' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 6 }}>
                          {Object.entries(THEMES).map(([k, th]) => (
                            <button key={k} onClick={() => updateP(ep.id, 'theme', k)} style={{ padding: 9, borderRadius: 9, border: ep.theme === k ? `2px solid ${th.a}` : '2px solid rgba(255,255,255,0.08)', background: th.bg, cursor: 'pointer', textAlign: 'left' }}>
                              <div style={{ fontSize: 12, fontWeight: 600, color: th.t }}>{th.n}</div><div style={{ width: 12, height: 12, borderRadius: '50%', background: th.a, marginTop: 4 }} />
                            </button>
                          ))}
                        </div>
                      </div>
                      <div>
                        ...
                        <label style={{ display: 'block', padding: 11, background: 'rgba(255,255,255,0.05)', borderRadius: 10, textAlign: 'center', cursor: 'pointer', border: '1px dashed rgba(255,255,255,0.2)', fontSize: 13 }}>🖼 Загрузить фон<input type="file" accept="image/*" onChange={readFile(v => updateP(ep.id, 'customBg', v))} style={{ display: 'none' }} /></label>
                        {ep.customBg && <>
      ...
      {confirmDel && (() => { const isProf = confirmDel.type === 'profile', nm = isProf ? findP(confirmDel.id)?.name : editBlock?.title; return (
        <div onClick={() => setConfirmDel(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, backdropFilter: 'blur(6px)' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#1a1a2e', padding: 28, borderRadius: 14, width: 320, border: '1px solid rgba(255,255,255,0.1)', textAlign: 'center' }}>
            <div style={{ fontSize: 36, marginBottom: 12 }}>🗑</div><h3 style={{ color: '#fff', marginBottom: 8, fontSize: 16 }}>Удалить {isProf ? 'профиль' : 'ссылку'}?</h3><p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 13, marginBottom: 20 }}>{nm}</p>
            <div style={{ display: 'flex', gap: 8 }}>
